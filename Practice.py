...
def soup_html(html):
    ...
    if nextPage:
        return movie_name_list, movie_common_list, movie_score_list, movie_link_list, DOWNLOAD_URL + nextPage['href']
    return movie_name_list, movie_common_list, movie_score_list, movie_link_list, None
def main():
    url = DOWNLOAD_URL
    with codecs.open('movies', 'wb', encoding='utf-8') as fp:
        while url:
            html = download_page(url)
            movies, common, score, link, url = soup_html(html)
            data_list = add_dict(movies, common, score, link)
            save_data(data_list)


def save_data(dict_list):
    client = pymongo.MongoClient('localhost', 27017)
    db_name = 'douban_movie_2'
    db = client[db_name]
...
def add_dict(movie_list, common_list, score_list, link_list):
    dict_list = []
    for i in range(0, movie_list.__len__()):
        list1 = ['Name', 'Common', 'Score', 'link']
        list2 = [movie_list[i], common_list[i], score_list[i], link_list[i]]
